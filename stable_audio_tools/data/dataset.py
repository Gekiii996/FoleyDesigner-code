...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for config in configs:
            print(f'dky config: {config.path}')
...
class FilmStereoDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        configs,
        sample_size=65536,
    ...
    def load_file(self, filename):
        ext = filename.split(".")[-1]
        if ext == "mp3":
            with torchaudio.backend.soundfile_backend.AudioFile(filename) as f:
                audio = f.read(f.frames)
                audio = torch.from_numpy(audio)
                in_sr = f.samplerate
        else:
    ...
    def __getitem__(self, idx):
        audio_filename = self.filenames[idx]
        # try:
        audio_path = audio_filename['path']
        motion_type = audio_filename.get('motion_type')
        movement_str = audio_filename.get('movement_str')
        start_points = audio_filename.get('start_points')
        end_points = audio_filename.get('end_points')
        prompt = audio_filename['prompt']

        audio,ori_sr = self.load_file(audio_path)
        audio, t_start, t_end, seconds_start, seconds_total, padding_mask = self.pad_crop(audio)



        num_samples = audio.shape[-1]
        pad_len = int((padding_mask == 0).sum().item())
        offset = int(t_start * (audio.shape[-1]))
        ...
        # 如果存在前端裁剪 offset，可以裁切掉前 offset 部分
        if offset > 0:
            pos_wave = pos_wave[offset:offset + num_samples]

        # 如果有 padding，则将 padding 段 mask 置 0
        if pad_len > 0:
            pos_wave[-pad_len:, 2] = 0.0

        pos_wave = torch.tensor(pos_wave) # [1, T, 3]


        info = {
            "path": audio_path,
            "prompt": prompt,
            "pos": pos_wave,  # 👈 加入位置控制信息
            "sample_rate": self.sr,
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4):
    ...
    if dataset_type == "audio_dir":
    ...
    elif dataset_type == 'film_stereo':

        film_stereo_configs = dataset_config.get("datasets", None)

        assert film_stereo_configs is not None, "Directory configuration must be specified in datasets[\"dataset\"]"

        configs = []

        for film_stereo_config in film_stereo_configs:

            item = {}
            film_stereo_path = film_stereo_config.get("path", None)
            assert film_stereo_path is not None, "Path must be set for FilmStereo directory configuration"
            prompt = film_stereo_config.get("prompt", "")



            item['prompt'] = prompt
            item['path'] = film_stereo_path
            item['motion_type'] = film_stereo_config['motion_type']
            item['movement_str'] = film_stereo_config['movement_str']
            item['start_points'] = film_stereo_config['start_points']
            item['end_points'] = film_stereo_config['end_points']

            configs.append(item)



        train_set = FilmStereoDataset(
            configs,
            sample_rate=sample_rate,
            sample_size=sample_size,
        ...
        return torch.utils.data.DataLoader(train_set, batch_size, shuffle=True,num_workers=num_workers, persistent_workers=True, pin_memory=True, drop_last=True, collate_fn=collation_fn)