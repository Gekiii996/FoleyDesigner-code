    ...
    def __init__(
        ...
        for i in range(depth, 0, -1):
            ...
            if i > 1:
                ...
                block = SkipBlock(
                    Downsample1d_2(c_prev, c_prev, stride) if (learned_resample or stride == 1) else Downsample1d(
                        "cubic"),
...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[str, tp.Any], negative=False):
        ...
        pos = conditioning_tensors.get('pos', None)
        if isinstance(pos, (list, tuple)) and len(pos) == 2:
            pos_embd, pos_mask = pos
        else:
            pos_embd, pos_mask = None, None
...
def create_diffusion_cond_from_config(config: tp.Dict[str, tp.Any]):
    ...
    model_type = config["model_type"]  # "diffusion_cond"

    diffusion_config = model_config.get('diffusion', None)
    assert diffusion_config is not None, "Must specify diffusion config"

    diffusion_model_type = diffusion_config.get('type', None)  # 'dit'
    ...
    diffusion_objective = diffusion_config.get('diffusion_objective', 'v')  # default v
    ...
    if conditioning_config is not None:
        # create conditioners prompt seconds_start seconds_total
    ...
    if pretransform is not None:
        pretransform = create_pretransform_from_config(pretransform, sample_rate)
        min_input_length = pretransform.downsampling_ratio  # 2048
    else:
        min_input_length = 1
