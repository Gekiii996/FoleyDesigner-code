        ...
        **kwargs):
        ...
        if cfg_scale != 1.0 and (cross_attn_cond is not None or prepend_cond is not None):
            ...
            if pos_embd is not None:
                # null version for unconditioned branch
                null_pos = torch.zeros_like(pos_embd, device=pos_embd.device)
                batch_pos_embd = torch.cat([pos_embd, null_pos], dim=0)

                if pos_mask is not None:
                    null_mask = torch.zeros_like(pos_mask, device=pos_mask.device)
                    batch_pos_mask = torch.cat([pos_mask, null_mask], dim=0)
                else:
                    batch_pos_mask = None
