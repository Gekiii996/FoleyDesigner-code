    ...
    def __init__(
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                print(f'[INFO] t5_model_name :{t5_model_name}')
                self.tokenizer = AutoTokenizer.from_pretrained('/root/.cache/huggingface/hub/models--t5-base/snapshots/a9723ea7f1b39c1eae772870f3b547bf6ef7e6c1')
                model = T5EncoderModel.from_pretrained('/root/.cache/huggingface/hub/models--t5-base/snapshots/a9723ea7f1b39c1eae772870f3b547bf6ef7e6c1').train(enable_grad).requires_grad_(enable_grad).to(torch.float16)
            finally:
                logging.disable(previous_level)
...
class PosConditioner(Conditioner):
    def __init__(
            self,
            output_dim: int,
            fourier_M: int = 64,
            latent_dim: int = 64,
            max_length: int = 2_097_152,  # Maximum sequence length for padding positions
            finetune: bool = False,
            project_out: bool = False
    ):
        super().__init__(latent_dim, output_dim, project_out=project_out)
        self.max_length = max_length
        self.finetune = finetune

        self.pos_embd = PosEmbd(fourier_M=fourier_M, latent_dim=latent_dim)
        self.proj_out = nn.Identity()
        if not finetune:
            for param in self.pos_embd.parameters():
                param.requires_grad = False
        self.pos_embd.eval()

    def forward(self, pos_list: tp.List[torch.Tensor], device: tp.Union[torch.device, str]) -> tp.Tuple[
        torch.Tensor, torch.Tensor]:
        # Find max T in batch, cap at max_length
        lengths = [p.shape[0] for p in pos_list]
        max_T = min(max(lengths), self.max_length)

        padded_pos = []
        batch_masks = []  # 真实 mask: 1 for real, 0 for padding

        for p, L in zip(pos_list, lengths):
            p = p.to(device).float()
            T = min(L, max_T)
            pad_len = max_T - T

            if pad_len > 0:
                pad_tensor = torch.zeros(pad_len, 3, dtype=p.dtype, device=device)
                pad_tensor[:, :2] = 0.0
                pad_tensor[:, 2] = 0.0  # mask = 0 for padding
                p = torch.cat([pad_tensor, p[:T]], dim=0)
            else:
                p = p[:T]
            ...
            ], dim=0) if pad_len > 0 else torch.ones(T, device=device)

            batch_masks.append(mask)

        pos = torch.stack(padded_pos, dim=0)  # [B, max_T, 3]
        mask = torch.stack(batch_masks, dim=0).bool()  # [B, max_T]

        with torch.no_grad() if not self.finetune else torch.enable_grad():
            latent = self.pos_embd(pos)  # [B, latent_dim, T_down]
            latent = latent.permute(0, 2, 1)  # [B, T_down, latent_dim]
        embeds = self.proj_out(latent)  # [B, T_down, output_dim]

        # Downsample mask: repeat or interpolate? We use nearest (same as input)
        # But since downsampling is not uniform, we take max in each window
        # Simple way: interpolate mask to T_down
        mask_down = F.interpolate(
            mask.float().unsqueeze(1),  # [B,1,max_T]
            size=embeds.shape[-1],
            mode='nearest'
        ).squeeze(1).bool()  # [B, T_down]

        return embeds, mask_down
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any]) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "pos":
            conditioners[id] = PosConditioner(**conditioner_config)
        else:
