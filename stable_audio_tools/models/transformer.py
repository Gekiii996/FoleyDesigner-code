...
class Attention(nn.Module):
    def __init__(
        ...
        self.use_fa_flash = torch.cuda.is_available()
...
class ContinuousTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        injection_layers_idx = [3, 7, 11, 15, 19, 23]
        self.injection_layers = nn.ModuleList([])
        self.pos_embed_norm = LayerNorm(64) 
        ...
        for i in range(depth):
            ...
            if i in injection_layers_idx:
                # self.injection_layers.append(
                #     TransformerBlock(
                #         dim=dim,
                #         dim_heads=dim_heads,
                #         cross_attend=True,
                #         dim_context=64,  # context 是 (B,1,64)
                #         global_cond_dim=global_cond_dim,  # injection 不走 adaLN
                #         causal=causal,
                #         zero_init_branch_outputs=zero_init_branch_outputs,
                #         conformer=conformer,
                #         layer_ix=-1,
                ...
                self.injection_layers.append(
                    nn.ModuleDict({
                        'attn': Attention(
                            ...
                            dim_heads=dim_heads,
                            dim_context=dim,
                            causal=False, # 交叉注意力通常不是因果的
                            zero_init_output=True, # 保持输出初始化为0，有助于稳定
                            **kwargs.get('attn_kwargs', {})
                        )
                    })
                )
            else:
                self.injection_layers.append(None)
    ...
    def forward(
        ...
        **kwargs
        ...
        for main_layer, inj_layer in zip(self.layers, self.injection_layers):
            # ---- 主 TransformerBlock ----
            x = checkpoint(main_layer, x, rotary_pos_emb = rotary_pos_emb, global_cond=global_cond, **kwargs)
            if return_info:
                info["hidden_states"].append(x)

            # if inj_layer is not None:
            #     if pos_embd is not None:
            #         normed_pos_embd = self.pos_embed_norm(pos_embd)
            #         x = checkpoint(
            #             inj_layer,
            #             x,
            #             context=normed_pos_embd,  # ← 关键：作为 context
            #             context_mask=pos_mask,
            #             mask=mask,  # query 的 mask
            #             rotary_pos_emb=rotary_pos_emb,
            #             global_cond=global_cond  # injection 不走 adaLN
            #         )
            ...
            if inj_layer is not None:
                if pos_embd is not None:
                    ...
                    # ---- 修改开始：调用轻量级模块 ----
                    # 使用残差连接
                    injection_output = inj_layer['attn'](
                        x, # 先对 x 做 norm
                        context=normed_pos_embd,
                        context_mask=pos_mask
                    )
                    x = x + injection_output # 直接将交叉注意力的结果加到 x 上
                    # ---- 修改结束 ----
                    if return_info:
                        info["hidden_states"].append(x)
